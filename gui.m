...
% Last Modified by GUIDE v2.5 24-Jul-2015 20:40:04
...
% --- Executes on button press in ButtonExplore.
function ButtonExplore_Callback(hObject, eventdata, handles)
  [handles.filename, handles.pathname] = uigetfile({'*.png;*.tif;*.bmp;*.jpg','Images';'*.*','All files' },'Select an image');
    ...
    if (~isempty(get(handles.EditTextURL, 'string')))
        ...
        axes(handles.axes_page);
        imshow(img);
        set(handles.axes_page,'menubar','figure');
        set(handles.axes_page,'toolbar','figure');
